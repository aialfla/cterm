...
<%-- 							<option value="1"<c:if test="${param.duty eq 1}">selected</c:if>>대표</option> --%>
			...
			<!-- == 목록 테이블 시작 ===================================================================== -->
			<table class="tb_list">
				<caption class="blind">초과근무 내역</caption>
				<thead>
					<tr>
				...
				<tbody>
				<c:forEach var="item" items="${ overlist }">
					<tr>
						<td class="msg_name">
						...
						<td class="tl msg_name">
							<a href="<%= request.getContextPath() %>/auth/view_over.do?overNO=${ item.overNO }">
								<!-- 사원용<문서 결재상태> -->
								<c:if test="${ loginUser.duty != 0 }">
									<c:choose>
							...
							<td>
								<c:choose>
									<c:when test="${ item.astate == 0 || item.astate == 1 }">
										<a href="<%= request.getContextPath() %>/auth/view_over.do?overNO=${ item.overNO }" class="astate_wait">
			...
			<!-- == 페이징 시작 ===================================================================== -->
			<c:if test="${ not empty overlist }">
			<div class="paging">
				<ul>
					<c:if test="startBlock > 1">
						<li>
							<a href="${pageContext.request.contextPath}/auth/list_over.do?pageNO=${ startBlock-1 }
								...
								title="처음 페이지로 이동" class="btn_first">
					...
					<li>
						<a href="${pageContext.request.contextPath}/auth/list_over.do?pageNO=${ pageNO-1 }
							<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
							<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
							<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
							<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
					...
					<c:forEach var="i" begin="${startBlock}" end="${endBlock}" step="1">
						<li>
							<a href="${pageContext.request.contextPath}/auth/list_over.do?pageNO=${ i }
								<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
								<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
								<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
								<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
					...
					<li>
						<a href="${pageContext.request.contextPath}/auth/list_over.do?pageNO=${ pageNO+1 }
							...
							title="다음 페이지로 이동" 
							<c:choose>
								<c:when test="${ pageNO == endBlock }">class="btn_next" onclick="return false;"</c:when>
								<c:when test="${ pageNO != endBlock }">class="btn_next active"</c:when>
							</c:choose>
						>
					...
					<c:if test="endBlock != MaxPage">
						<li>
							<a href="${pageContext.request.contextPath}/auth/list_over.do?pageNO=${ endBlock+1 }
								<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
								<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
								<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
								<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
								<c:if test="${ not empty param.name}">&name=${param.name}</c:if>" 
								title="끝 페이지로 이동" class="btn_last">
