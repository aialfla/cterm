...
<body>
	...
	<main id="main">
		...
		<section id="main_warp">
			...
			<!-- == 서브타이틀 시작 ===================================================================== -->
			<form action="list_docu.do" method="get">
			<div class="tit_sub">
				<div class="date_warp">
...
<%-- 							<option value="1"<c:if test="${param.duty eq 1}">selected</c:if>>대표</option> --%>
			...
			<table class="tb_list">
				...
				<thead>
					<tr>
						...
						<th class="col_w15">결재상태</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach var="item" items="${ doculist }">
						<tr>
							<td class="msg_name">
							...
							<td class="tl msg_name">
								<a href="<%= request.getContextPath() %>/auth/view_docu.do?docuNO=${ item.docuNO }">
									<!-- 사원용<문서 결재상태> -->
									<c:if test="${ loginUser.duty != 0 }">
										<c:choose>
								...
								<td>
									<c:choose>
										<c:when test="${ item.astate == 0 || item.astate == 1 }">
											<a href="<%= request.getContextPath() %>/auth/view_docu.do?docuNO=${ item.docuNO }" class="astate_wait">
			...
			<!-- == 페이징 시작 ===================================================================== -->
			<c:if test="${ not empty doculist }">
			<div class="paging">
				<ul>
					<c:if test="${ MaxPage > 10 }">
						<li>
							<a href="${pageContext.request.contextPath}/auth/list_docu.do?pageNO=1
					...
					<li>
						<a href="${pageContext.request.contextPath}/auth/list_docu.do?pageNO=${ pageNO-1 }
							<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
							<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
							<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
							<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
					...
					<c:forEach var="i" begin="${startBlock}" end="${endBlock}" step="1">
						<li>
							<a href="${pageContext.request.contextPath}/auth/list_docu.do?pageNO=${ i }
								<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
								<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
								<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
								<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
					...
					<c:if test="${ MaxPage > 10 }">
						<li>
							<a href="${pageContext.request.contextPath}/auth/list_docu.do?pageNO=${ MaxPage }
								<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
								<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
								<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
								<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
