...
<body>
	...
	<main id="main">
		...
		<section id="main_warp">
			...
			<!-- == 서브타이틀 시작 ===================================================================== -->
			<form method="Get" action="list_mine.do">
			<div class="tit_sub">
				<div class="date_warp">
				...
				<a href="<%= request.getContextPath() %>/docu/write.do" class="btn_write"><i class="fi fi-sr-file-edit"></i>기안 작성</a>
			</div>
			</form>
			<!-- == 서브타이틀 종료 ===================================================================== -->
			...
			<div class="docu_warp">
				<table class="tb_list">
					<caption class="blind">내가 작성한 기안</caption>
					<thead>
						<tr>
							...
							<tr>
								...
								<td>
									<c:choose>
										...
										<c:when test="${ item.state eq 9 }"><span class="state_back">철회</span></c:when>
									</c:choose>
								</td>
							</tr>
						</c:forEach>
			...
			<div class="paging">
				<ul>
					<c:if test="${ MaxPage > 10 }">
						<li>
							<a href="${pageContext.request.contextPath}/docu/list_mine.do?pageNO=1
					...
					<li>
						<a href="${pageContext.request.contextPath}/docu/list_mine.do?pageNO=${ pageNO-1 }
							<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
							<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
							<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
							<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
					...
					<c:forEach var="i" begin="${startBlock}" end="${endBlock}" step="1">
						<li>
							<a href="${pageContext.request.contextPath}/docu/list_mine.do?pageNO=${ i }
								<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
								<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
								<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
								<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
					...
					<c:if test="${ MaxPage > 10 }">
						<li>
							<a href="${pageContext.request.contextPath}/docu/list_mine.do?pageNO=${ MaxPage }
								<c:if test="${ not empty param.startday}">&startday=${param.startday}</c:if>
								<c:if test="${ not empty param.endday}">&endday=${param.endday}</c:if>
								<c:if test="${ not empty param.dept}">&dept=${param.dept}</c:if>
								<c:if test="${ not empty param.duty}">&duty=${param.duty}</c:if>
